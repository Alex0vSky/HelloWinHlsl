...
template<> class ball<DxVer::v9> : public CurClientApp<DxVer::v9> {
	using T = DxVer::v9;
	...
	bool init(DxCtx<T>::cref_ptr_t crpustDxCtx, ToolCtx<T>::cref_ptr_t puoTools, Adjust<T>* poAdjustDxAux) {
		...
		m_pcVs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Vs( 
				[]() ->const auto & {
					static 
#include "resource\ball_vs_Dx9.hlsl.h"
					return g_main;
				}
			);
		m_pcPs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Ps( 
				[]() ->const auto & {
					static 
#include "resource\ball_ps_Dx9.hlsl.h"
					return g_main;
				}
			);
		if ( !m_pcVs || !m_pcPs )
		...
		bool bRv = m_puPsConstSetter ->setFloat4( "ve4Resolution", D3DXVECTOR4{ (float)stPp.BackBufferWidth, (float)stPp.BackBufferHeight, 0, 0 } );
...
template<> class ball<DxVer::v10> : public CurClientApp<DxVer::v10> {
	using T = DxVer::v10;
	...
	bool init(DxCtx<T>::cref_ptr_t crpustDxCtx, ToolCtx<T>::cref_ptr_t puoTools, Adjust<T>* poAdjustDxAux) {
		...
		m_pcVs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Vs( 
				[]() ->const auto & {
					static 
#include "resource\ball_vs_Dx10.hlsl.h"
					return g_main;
				}
				, &veShaderByte
			);
		m_pcPs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Ps( 
				[]() ->const auto & {
					static 
#include "resource\ball_ps_Dx10.hlsl.h"
					return g_main;
				}
			);
		if ( !m_pcVs || !m_pcPs )
		...
		crpustDxCtx ->m_pcD3dDevice ->IASetVertexBuffers( 0, stVBuf ->m_stVertexBuf.c_uNumBuffers, stVBuf->m_stVertexBuf.m_veBuffers.data( ), stVBuf->m_stVertexBuf.m_veStride.data( ), stVBuf->m_stVertexBuf.m_veOffset.data( ) );
		// Input assembly
	...
	void render_frame(DxCtx<T>::cref_ptr_t crpustDxCtx, Dynamic<T>::cref_ptr_t) {
		...
		crpustDxCtx ->m_pcD3dDevice ->ClearRenderTargetView( crpustDxCtx ->m_pcD3dRenderTargetView.Get( ), ClearColor );
...
template<> class ball<DxVer::v11> : public CurClientApp<DxVer::v11> {
	using T = DxVer::v11;
	...
	bool init(DxCtx<T>::cref_ptr_t crpustDxCtx, ToolCtx<T>::cref_ptr_t puoTools, Adjust<T>* poAdjustDxAux) {
		...
		m_pcVs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Vs( 
				[]() ->const auto & {
					static 
#include "resource\ball_vs_Dx11.hlsl.h"
					return g_main;
				}
				, &veShaderByte
			);
		m_pcPs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Ps( 
				[]() ->const auto & {
					static 
#include "resource\ball_ps_Dx11.hlsl.h"
					return g_main;
				}
			);
		if ( !m_pcVs || !m_pcPs )
		...
		if ( !stVBuf )
			return false;
		m_pcVertexBuf = stVBuf ->m_stVertexBuf.m_pcBuffer;
		crpustDxCtx ->m_pcDeviceContext ->IASetVertexBuffers( 0, stVBuf ->m_stVertexBuf.c_uNumBuffers, stVBuf->m_stVertexBuf.m_veBuffers.data( ), stVBuf->m_stVertexBuf.m_veStride.data( ), stVBuf->m_stVertexBuf.m_veOffset.data( ) );
		// Input assembly
	...
	void render_frame(DxCtx<T>::cref_ptr_t crpustDxCtx, Dynamic<T>::cref_ptr_t) {
		float ClearColor[4] = { 0.0f, 0.25f, 0.25f, 0.55f };
		crpustDxCtx ->m_pcDeviceContext ->ClearRenderTargetView( crpustDxCtx ->m_pcD3dRenderTargetView.Get( ), ClearColor );
...
template<> class ball<DxVer::v12> : public CurClientApp<DxVer::v12> {
	using T = DxVer::v12;
	LikeDxut::Timer m_oTimer;
	...
	bool init(DxCtx<T>::cref_ptr_t crpustDxCtx, ToolCtx<T>::cref_ptr_t puoTools, Adjust<T>* poAdjustDxAux) {
		...
		m_pcVs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Vs( 
				[]() ->const auto & {
					static 
#include "resource\ball_vs_Dx12.hlsl.h"
					return g_main;
				}
			);
		m_pcPs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Ps( 
				[]() ->const auto & {
					static 
#include "resource\ball_ps_Dx12.hlsl.h"
					return g_main;
				}
			);
		if ( !m_pcVs || !m_pcPs )
	...
	void render_frame(DxCtx<T>::cref_ptr_t crpustDxCtx, Dynamic<T>::cref_ptr_t crpsoDynamic) {
...
uptr< Sys::HolderClientApp > ClientTy::entryPoint() { return std::make_unique< Sys::HolderClientApp >( new ball<CurDxVer>( ) ); }
