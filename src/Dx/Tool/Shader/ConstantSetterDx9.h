...
class ConstantSetterDx9 {
 ...
 public:
	...
	class Setter {
		const Ty::StDxCtx_ptr<TInnerDxVer> m_stDxCtx;
		const CPtr< ID3DXConstantTable > m_pcConstantTable;
		
	public:
		typedef uptr< Setter > uptrc_t;
		Setter(Ty::StDxCtx_crefPtr<TInnerDxVer> stDxCtx
				, const CPtr<ID3DXConstantTable> &pcConstantTable
			)
			: m_stDxCtx( stDxCtx )
			, m_pcConstantTable( pcConstantTable )
		 {}
		bool setFloat4(const std::string &strNameConst, const D3DXVECTOR4 &value) const {
			...
			if ( D3DXHANDLE h = m_pcConstantTable ->GetConstantByName( 0, strNameConst.c_str( ) ) )
				if ( SUCCEEDED( m_pcConstantTable ->SetVector( m_stDxCtx ->m_pcD3dDevice.Get( ), h, &value ) ) ) 
					return true;
			return false;
		}
		bool setMatrix(const std::string &strNameConst, const D3DXMATRIX &value) const {
			if ( D3DXHANDLE h = m_pcConstantTable ->GetConstantByName( 0, strNameConst.c_str( ) ) )
				if ( SUCCEEDED( m_pcConstantTable ->SetMatrix( m_stDxCtx ->m_pcD3dDevice.Get( ), h, &value ) ) ) 
					return true;
			return false;
		}
		bool setFloat(const std::string &strNameConst, float value) const {
			if ( D3DXHANDLE h = m_pcConstantTable ->GetConstantByName( 0, strNameConst.c_str( ) ) )
				if ( SUCCEEDED( m_pcConstantTable ->SetFloat( m_stDxCtx ->m_pcD3dDevice.Get( ), h, value ) ) ) 
					return true;
			return false;
		}
	};
	...
	create(const CPtr<U> &cp) const {
		...
		if ( FAILED( cp ->GetFunction( &veMemFunc[ 0 ], &uSizeData ) ) ) 
			return nullptr;
		CPtr< ID3DXConstantTable > pcConstantTable;
		if ( FAILED( ::D3DXGetShaderConstantTable( 
				reinterpret_cast<DWORD *>( &veMemFunc[ 0 ] )
				, pcConstantTable.ReleaseAndGetAddressOf( ) ) ) 
			) 
			return nullptr;
		return std::make_unique< Setter >( m_stDxCtx, pcConstantTable );
	}
};
} // namespace prj_3d::HelloWinHlsl::Dx::Tool::Shader
