// Dx/ConstantBuf/viaStruct/FactoryPackoffsetAware - factory for deduce 
//	type of template wrapper, with input struct type checker
// TODO(Alex0vSky): finishme
...
class FactoryPackoffsetAware {
 ...
 public:
	explicit FactoryPackoffsetAware(const Dx::Ctxs &stCtx) 
		: m_stCtx( stCtx ) 
	 {}
	...
	auto createAccessor() {
		// This will no longer be necessary, we will allocate ourselves: 
		//	static_assert( (sizeof( TConstBuf ) % 16) == 0, "size not padded correctly" );
		...
		boost::pfr::for_each_field(TConstBuf{} , [&veSizeof]
			(const auto& field, auto qwe) {
				...
				//	it is also possible here it is worth checking the output 
				//	of the last field for 16 bytes, although it is unlikely.
			} );
