// DrawAux/Fps/Spec/D12/ResourceUploadBatch.h - from DirectXTK
#pragma once // NOLINT copyright
namespace prj_3d::HelloWinHlsl::DrawAux::Fps::Spec::D12 {

typedef SharedGraphicsResource SharedGraphicsResource_t;
...
#include "GenerateMips_main.inc"
    ...
    private:
        static CPtr<ID3D12RootSignature> CreateGenMipsRootSignature(
            _In_ ID3D12Device* device)
        {
        ...
        static CPtr<ID3D12PipelineState> CreateGenMipsPipelineState(
            _In_ ID3D12Device* device,
...
public:
        ...
        uint32_t numSubresources)
    {
        ...
        CPtr<ID3D12Resource> scratchResource = nullptr;
...
private:
        ...
        _In_ ID3D12Resource* resource)
    {
        ...
        CPtr<ID3D12DescriptorHeap> descriptorHeap;
        ...
        CPtr<ID3D12PipelineState> pso = mGenMipsResources->generateMipsPSO;
        ...
        _In_ ID3D12Resource* resource)
    {
        ...
        CPtr<ID3D12Resource> resourceCopy;
        ThrowIfFailed(mDevice->CreateCommittedResource(
            &heapProperties,
            D3D12_HEAP_FLAG_NONE,
            &copyDesc,
    ...
    struct UploadBatch
    {
        std::vector<CPtr<ID3D12DeviceChild>>  TrackedObjects;
        std::vector<SharedGraphicsResource_t>     TrackedMemoryResources;
    ...
    std::vector<CPtr<ID3D12DeviceChild>>      mTrackedObjects;
    std::vector<SharedGraphicsResource_t>         mTrackedMemoryResources;
