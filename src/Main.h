...
class Main {
	...
	int mainTpl(HINSTANCE hInst, config_t::cref_ptr_t pustCfg, uptr< Sys::HolderClientApp > pustHolder) {
		...
		// Drawing by the client on a prepared DirectX3D context and the library outputting the frame-limited FPS value.
...
public:
	...
	int run(HINSTANCE hInst) {
		...
		// TODO: try catch and Sys::ErrorHandler::showMsgAndExit( "exception" );
